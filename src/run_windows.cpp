...
int main()
{
    ...
    read_wts_biases("wts_biases_new_Relu_softmax.bin", weights, biases);
        ...
        { // Enter key
            if (!croppedImage.empty())
            {
                ...
                if (isSaved)
                {
                    ...
                    cv::Mat blurredImage;
                    cv::GaussianBlur(thresholdedImage, blurredImage, cv::Size(5, 5), 1);
                    ...
                    int extendedSize = size + 2;  //2 pixels padding(1 pixel border on each side)
                    cv::Mat squareCanvas = cv::Mat::zeros(extendedSize, extendedSize, blurredImage.type());

                    int x_offset = (extendedSize - boundingBox.width) / 2 ;  
                    int y_offset = (extendedSize - boundingBox.height) / 2 ; 


                    blurredImage.copyTo(squareCanvas(cv::Rect(x_offset, y_offset, boundingBox.width, boundingBox.height)));
                    cv::imshow("ROI extraction with centering maintaining aspect ratio", squareCanvas);


                    key = cv::waitKey(0);

                    cv::Mat downscaledImage;
                    cv::resize(squareCanvas, downscaledImage, cv::Size(28, 28), 0, 0, cv::INTER_AREA);
					...
					key = cv::waitKey(0);
                    cv::namedWindow("Zoomed Downscaled Image", cv::WINDOW_NORMAL); // Create window with adjustable size
                    cv::resizeWindow("Zoomed Downscaled Image", 560, 560); // Resize the window to your desired zoom level
                    cv::imshow("Zoomed Downscaled Image", expandedImage);
