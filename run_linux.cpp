...
void softmax(std::vector<float> &z) {
	...
	// First loop: Compute exponentials and the sum of exponentials
	// (denominator)
	for (auto &j : z) {
...
int main() {
	...
	read_wts_biases("wts_biases_new.bin", weights, biases);
	...
	while (true) {
		...
		if (key == 13) { // Enter key
			if (!croppedImage.empty()) {
				...
				if (isSaved) {
					std::cout << "Cropped image saved as " << outputFilename
							  << std::endl;

					cv::Mat grayscaleCropped;
					cv::cvtColor(croppedImage, grayscaleCropped,
								 cv::COLOR_BGR2GRAY);
					...
					// binary thresholding to get a black digit on a white
					// background
					cv::Mat thresholdedImage;
					cv::threshold(contrastEnhanced, thresholdedImage, 100, 255,
								  cv::THRESH_BINARY_INV + cv::THRESH_OTSU);
					cv::imshow("Thresholded Image (white on black)",
							   thresholdedImage);

					key = cv::waitKey(0);

					cv::Mat blurredImage;
					cv::GaussianBlur(thresholdedImage, blurredImage,
									 cv::Size(5, 5), 1);
					cv::imshow("Blurred Image", blurredImage);

					key = cv::waitKey(0);

					// ROI extraction ko lagi
					int size = std::max(boundingBox.width, boundingBox.height);

					cv::Mat squareCanvas =
						cv::Mat::zeros(size, size, blurredImage.type());

					int x_offset = (size - boundingBox.width) /
								   2; // 1 pixel extra (for padding)
					int y_offset = (size - boundingBox.height) / 2;

					blurredImage.copyTo(squareCanvas(
						cv::Rect(x_offset, y_offset, boundingBox.width,
								 boundingBox.height)));
					cv::imshow("ROI extraction with centering maintaining "
							   "aspect ration and 2-pixel padding",
							   squareCanvas);
					...
					cv::resize(squareCanvas, downscaledImage, cv::Size(24, 24),
							   0, 0, cv::INTER_AREA);
					cv::imshow("Downscaled Image", downscaledImage);
					key = cv::waitKey(0);

					cv::Mat expandedImage;
					cv::copyMakeBorder(downscaledImage, expandedImage, 2, 2, 2,
									   2, cv::BORDER_CONSTANT,
									   cv::Scalar(0, 0, 0));


					cv::imshow("Padded Image", expandedImage);

					key = cv::waitKey(0);

					// save the downscaled image
					std::string downscaledFilename = "downscaled_digit.png";
					bool isDownscaledSaved =
						cv::imwrite(downscaledFilename, expandedImage);
					if (isDownscaledSaved) {
						std::cout << "Downscaled image saved as "
								  << downscaledFilename << std::endl;
					} else {
						std::cerr
							<< "Error: Could not save the downscaled image!"
							<< std::endl;
					}

					std::vector<float> input_data =
						prepare_input_image(downscaledFilename);

					print_vec_result(input_data);

					auto result = custom_test(input_data, 784, 10, weights,
											  biases, 2, 16);

					int predicted_digit = 0;
					float loss;
					for (int j = 0; j < 10; j++) {
						if (result[j] > result[predicted_digit])
							predicted_digit = j;
					}

					loss = -log(result[predicted_digit]);

					std::cout << "Predicted digit: " << predicted_digit << "\tError: " << loss
							  << std::endl;
					...
					std::string text =
						"Predicted digit: " + std::to_string(predicted_digit);
					...
					cv::putText(image, text, position, font, fontScale, color,
								thickness);
					...
					std::cerr << "Error: Could not save the cropped image!"
							  << std::endl;
				}
			} else {
